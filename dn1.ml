...
let rec collatz n = (
  ...
  | _ ->
      if n mod 2 = 0 then
        n :: collatz (n / 2) (*Doda na začetek sezna če je sodo, isto spodaj za liho*)
      else
        n :: collatz (3 * n + 1)
);;
...
let fiksne_tocke f list = List.filter (fun x -> f x = x) list;;
...
let rec sep_concat n x = 
  match x with
  | [] -> [n]
  | [g] -> n :: g @ [n]
  | g :: r -> n :: g @ sep_concat n r


(* Tole se bi dalo tudi s fold left napisat kot sem spodaj (ampak je verjetno z rekurzijo lažje)*)

(*let sep_concat n x = 
  match x with
  | [] -> [n]
  | _ -> List.fold_left (fun acc y -> acc @ (y @ [n]) ) [n] x*)

(*Tukaj za naš akumulator nesemo zraven cel list, kar je malo neučnikovitno...*)
...
let partition k sez = 
  let (seznami, trenutni) =  (* sabo bomo nesli množico seznamov in trenutni seznam*)
    List.fold_left(fun (seznami, curr) x ->
     let curr = x :: curr in (* vedno dodamo nov element na trenutni seznam*)
      if List.length(curr) = k then
        (List.rev curr :: seznami, []) (* ko imamo dovolj velik seznam ga dodamo kot glavo na sezname*)
      else
        (seznami, curr)
    ) ([], []) sez
    in
    let seznami = if trenutni = [] then seznami else trenutni :: seznami (* če nam ostane še kakšen seznam. ga dodamo tukaj*)
    in
    List.rev seznami  (* ker smo pushali back sezname, moramo seznam obrnit*)
...
let inverz x = 
    let rec najdi_indeks curr value idx =
      match curr with
      | [] -> 0
      | h :: t -> if h = value then idx else najdi_indeks t value (idx + 1)
    in
    let rec ustvari_inverz i = 
      if i >= List.length x then []
      else
        let indeks = najdi_indeks x i 0 in
        indeks :: ustvari_inverz (i + 1)
    in
    ustvari_inverz 0 


let primer_permutacije_3 = inverz permutacija_1
(* val primer_permutacije_3 : int list = [5; 3; 2; 1; 4; 0] za [5; 3; 2; 1; 4; 0]*)
...
let cikli perm = (
    ...
    let rec naredi_cikel obiskani i =
      (*if List.mem i obiskani then ([], obiskani) else*)
      let rec aux j acc obiskani =
        if List.mem j obiskani then (List.rev acc, obiskani)  
        else
          let obiskani = j :: obiskani in
          let nov_indeks = List.nth perm j in

          aux nov_indeks (j :: acc) obiskani
      in
      aux i [] obiskani
    in
    let rec pojdi_cez_permutacijo i obiskani acc =
      match i, List.mem i obiskani with
      | i, _ when i >= n -> List.rev acc
      | _, true -> pojdi_cez_permutacijo (i + 1) obiskani acc
      | _, false ->
          let (cikel, obiskani') = naredi_cikel obiskani i in
          pojdi_cez_permutacijo (i + 1) obiskani' (cikel :: acc)
    in
    pojdi_cez_permutacijo 0 [] []
)
...
let iz_transpozicij n seznam = 
    let ustvari_permutacijo (a, b) =        (* Izkaže se, da bi lahko že ves ta čas uporabljal arrayje :)*)
    ...
    List.fold_left (fun acc x -> 
      kompozitum acc (ustvari_permutacijo x)
    ) identiteta seznam
...
let v_transpozicije permutacija = (
    let disjunktni_cikli= cikli permutacija
    in
    List.fold_left(fun acc x ->
      let rec aux j sez =
        if j >= List.length sez then []
        else
          (List.nth sez 0, List.nth sez j) :: aux (j + 1) sez
      in
      if List.length x = 1 then acc
      else
        acc @ aux 1 x
    ) [] disjunktni_cikli
...
let dodaj i j n m = 
  Array.mapi(fun i' vrstica ->
    if i' = i then
      Array.mapi (fun j' element -> if j' = j then Some n else element) vrstica
    else
      Array.copy(vrstica);  
  ) m
...
let izpis_mreze mreza = (
  let dobi_vrstico vrstica = 
      let rec cez_vrstico j =
        ...
        match j with
        | 8 -> element ^ " |"
        | 0 -> "|" ^ element ^ cez_vrstico (j+1)
        | _ -> match j mod 3 with
              | 0 -> " |" ^ element ^ cez_vrstico (j+1);
              | _ -> element ^ cez_vrstico (j+1);
      in
      cez_vrstico 0 ^ "\n"
    in
    let rec cez_stolpec i =
      let element = mreza.(i) in
      match i with
      | 8 -> dobi_vrstico(element) ^ "+-------+-------+-------+"
      | _ -> match i mod 3 with
            | 0 -> "+-------+-------+-------+\n" ^ dobi_vrstico(element) ^ cez_stolpec (i+1);
            | _ -> dobi_vrstico(element) ^ cez_stolpec (i+1);
    in
    cez_stolpec 0
)
...
let izpis_resitve resitev = (
  let mreza = Array.map( fun vrstica ->
    Array.map (fun element ->
      Some element) vrstica
  ) resitev
  in
  izpis_mreze mreza
)
